...
Все бесплатные инструменты требуют регистрации в ЛК → upsell в Smart Widget.

> Подробно: [`docs/BUSINESS_CONTEXT.md`](docs/BUSINESS_CONTEXT.md) | [`docs/PLG_STRATEGY.md`](docs/PLG_STRATEGY.md)

### Бизнес-контекст (из брифа)
- **Конкуренты РФ:** Carrot quest, Jivo, Envybox — наше отличие: AI-first, не шаблонные триггеры
- **Tone of voice:** дружелюбный и простой (ориентир — Tilda). Влияет на все тексты в UI
- **Монетизация:** Smart Widget = 7 дней триал → подписка. Планы: Free / Pro / Enterprise
- **Принцип виджета:** молчит, когда всё ок. Включается только при проблеме (уход, замешательство)
- **Айдентика:** в разработке (логотип не утверждён)
- **Будущие инструменты:** генератор пользовательского соглашения, согласие на ПД, 10+ других
...
**Общий принцип:** минималистично, но премиально. Чистый UX/UI с subtle-эффектами, не скучный flat.

| Правило | Суть |
|---------|------|
| Fixed/sticky элементы справа | Запрещено в инструментах (зарезервировано под Smart Widget) |
| Хардкод цветов (bg-blue-500) | Запрещено в инструментах/ЛК → семантические токены |
| backdrop-blur, subtle градиенты | **Разрешены везде** для функциональных элементов (модалки, поповеры, hover) |
...
- **НЕ использовать Claude in Chrome автоматически** — пользователь проверяет визуально сам. Только по прямой просьбе ("посмотри сам", "проверь визуально")
